  ...
  task :seed_parking_lots_availability => :environment do |task, args|
    ...
    if status
      ...
      lot_availabilities.each do |lot_availability|
        ...
        LotAvailability.find_or_create_by(parking_lot_id: parking_lot.id) do |pla|
          pla.address = parking_lot.address
          pla.latitude = parking_lot.latitude
          pla.longitude = parking_lot.longitude
          pla.total_lots = lot_availability['carpark_info'].inject(0) { |sum, hash| sum + hash['total_lots'].to_i }
          pla.lots_available = lot_availability['carpark_info'].inject(0) { |sum, hash| sum + hash['lots_available'].to_i } 
        end
      end
    else
